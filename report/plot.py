...
def _plot_ic(ax, ic):
    """图13（现金 --factor 专属）：IC 时间序列——每期 IC 柱(左轴) + 累计 IC 线(右轴)。

    ic: factor_test.compute_factor_ic 返回 dict（ic_series/ic_cum/ic_mean/ic_ir_annual/ic_t/ic_winrate）。
    """
    s = ic["ic_series"]
    ax.bar(s.index, s.values, width=15, color=C_STRATEGY, alpha=0.5, label="每期 IC")
    ax.axhline(0, color="#666666", lw=0.8)
    ax.set_ylabel("IC", fontsize=8)
    ax2 = ax.twinx()
    ax2.plot(ic["ic_cum"].index, ic["ic_cum"].values, color=C_UP, lw=1.4, label="累计 IC")
    ax2.set_ylabel("累计 IC", fontsize=8)
    ax2.spines["top"].set_visible(False)
    ax.set_title(
        f"IC 时间序列（IC均值={ic['ic_mean']:.3f}  ICIR年化={ic['ic_ir_annual']:.2f}  "
        f"t={ic['ic_t']:.2f}  IC胜率={ic['ic_winrate']*100:.0f}%）",
        fontsize=11, fontweight="bold", color=C_STRATEGY)
    _style_ax(ax)
...
def _plot_metrics_table(ax, metrics):
    ...
    # IC 标量（仅现金 --factor 路径注入 rd.metrics 时出现；权重路径无这些键 → 不渲染）
    for k in ("IC均值", "ICIR(年化)", "RankIC均值", "IC_t", "IC胜率"):
        if k in metrics:
            v = metrics[k]
            if k == "IC胜率":
                rows.append((k, f"{v*100:.1f}%"))
...
def _render_dashboard(rd: ReportData, save_dir: str = ".", title: str = "回测分析报告",
                      dashboard_name: str = "回测仪表盘.png", ic: dict = None) -> str:
    """ReportData → 一页仪表盘大拼图（11 图 + KPI）+ 持仓分析图 + 关键单图，存 save_dir。返回大图路径。

    ic: factor_test.compute_factor_ic 的 dict 或 None。非 None 时大图加第 5 行 IC 时间序列（现金 --factor 路径用）；
        None 时为原 4 行布局（权重引擎路径，逐点不变）。
    """
    _apply_style()
    metrics = rd.metrics
    ...
    # ── 仪表盘大拼图：4 行 × 3 列（ic 非空则加第 5 行 IC，整行铺满）──
    n_rows = 5 if ic is not None else 4
    fig = plt.figure(figsize=(18, 26 if ic is not None else 22))
    gs = gridspec.GridSpec(n_rows, 3, figure=fig, hspace=0.35, wspace=0.25,
    ...
    if ic is not None:
        _plot_ic(fig.add_subplot(gs[4, :]), ic)
...
def plot_cash_dashboard(result, save_dir: str = ".", title: str = "现金回测仪表盘",
                        periods_per_year: int = 252, rolling_window: int = 252,
                        ic: dict = None) -> str:
    ...
    输入: result — run_cash_backtest 返回的 BacktestResult；save_dir/title/年化天数/滚动窗。
      ic — factor_test.compute_factor_ic 的 dict 或 None。非 None（--factor 路径）→ 指标表加 5 个 IC 标量、
           大图加第 5 行 IC 时间序列；None（--weights 路径无因子）→ 不含 IC，与原仪表盘一致。
    ...
    if ic is not None:
        # IC 标量并进 rd.metrics 供指标表渲染（_plot_metrics_table 按可选键渲染，不在则跳过）
        rd.metrics = {**rd.metrics,
                      "IC均值": ic["ic_mean"], "ICIR(年化)": ic["ic_ir_annual"],
                      "RankIC均值": ic["rankic_mean"], "IC_t": ic["ic_t"], "IC胜率": ic["ic_winrate"]}
    return _render_dashboard(rd, save_dir, title, dashboard_name="现金回测仪表盘.png", ic=ic)
