...
# 导入引擎 + 分析层（把 backtest 根加到 sys.path，命名空间包导入，不建 __init__.py）
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
sys.path.insert(0, str(Path(__file__).resolve().parent.parent / "analysis"))
from engine.backtest import run_backtest, calc_benchmark  # noqa: E402
from metrics import calc_metrics  # noqa: E402  分组绩效复用统一指标，不再自带 _nav_metrics
...
def load_calendar() -> pd.DatetimeIndex:
    ...
    path = CACHE / "calendar" / "trade_dates.parquet"
    if not path.exists():
...
def load_price_df(codes, start, end, need_feasibility=False) -> pd.DataFrame:
    ...
    if need_feasibility:
        deriv = []
        for year in range(start.year, end.year + 1):
            f = CACHE / "derivative" / f"{year}.parquet"
            if not f.exists():
...
def load_index_eod(index_code: str) -> pd.DataFrame:
    ...
    path = CACHE / "index_eod" / f"{index_code.replace('.', '_')}.parquet"
    if not path.exists():
...
def load_index_members(index_code: str) -> pd.DataFrame:
    ...
    path = CACHE / "index_members" / f"{index_code.replace('.', '_')}.parquet"
    if not path.exists():
        have = sorted(p.stem for p in (CACHE / "index_members").glob("*.parquet"))
...
def load_st_intervals() -> pd.DataFrame:
    ...
    path = CACHE / "st" / "st_intervals.parquet"
    if not path.exists():
...
# ============================================================
# 通用区间 → 面板（指数成分股 / ST 共用，DRY）
# ============================================================
def intervals_to_panel(intervals, start_col, end_col, rebalance_dates, codes, end_inclusive) -> pd.DataFrame:
    """区间表 → (调仓日 × code) 布尔面板。

    code 在调仓日 D 命中任一区间 [start, end] 则 True；end 为 NaT 视 +∞。
    end_inclusive=True → start≤D≤end（ST）；False → start≤D<end（指数成分，退出日当天不再算）。
    实现：按 code 遍历其区间（区间数千级，非 hot loop）。
    """
    codes = list(codes)
    panel = pd.DataFrame(False, index=rebalance_dates, columns=codes)
    rd = rebalance_dates.values.astype("datetime64[ns]")
    code_pos = set(codes)
    iv = intervals[intervals["code"].isin(code_pos)]
    for code, grp in iv.groupby("code"):
        hit = np.zeros(len(rebalance_dates), dtype=bool)
