...
理想态(摩擦全 0、不开过滤)时,管线退化为"直接换到目标权重",净值逐点等于无摩擦基准。

三条线的关系:**当目标票全程可交易时**,`理想态净值 ≥ 含摩擦净值 ≥ 现金约束净值`,差距就是每层摩擦的代价。注意这不是恒等式——停牌/涨跌停/整手取整会迫使含摩擦、现金引擎被动持有现金;在下跌市,这部分被动现金少亏,可能让含摩擦甚至现金净值**反超**理想净值,此时"超额"部分来自被动空仓而非 alpha,解读需谨慎。
