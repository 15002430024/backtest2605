...
# 非对称下界：单日复权价跌到原值的 1% 以下（−99%/日）物理不可能，即便退市整理/长停复牌也到不了
# （真实最惨如 600766.SH 退市复牌 −96.4%，ratio≈0.036，仍在此线之上而被保留）。设这条线是为了抓
# "末日幻影低价无回跳"那类脏数据：上跳判据靠"跳回真实价"那天的 >10 倍抓，但若幻影低价正好是该股
# 缓存最后一天（其后退市/无数据），没有回跳可抓 → 不设下界就会放它进 NAV，把净值静默砸到 ~0。
MIN_DAILY_RATIO = 0.01
...
def validate_adjclose_quality(px, on_bad=None, max_daily_ratio=MAX_DAILY_RATIO,
                              min_daily_ratio=MIN_DAILY_RATIO):
    """检出物理不可能的单日复权价跳变并按 on_bad 处置。

    px: long [code, date, adj_close, ...]，已按 [code, date] 排序。
    判据(逐股按日)：adj_close ≤ 0，或相邻交易日比值 > max_daily_ratio(单日涨 >10 倍)，
    或比值 < min_daily_ratio(单日跌 >99%)。每股首日(无前值)不判，避开 IPO 首日无涨跌幅限制。
    [修1] 下界用【非对称】极端线 min_daily_ratio=0.01，不是 1/max_daily_ratio：真实退市整理/长停
    复牌当天可合法 −90%~−97%(如 600766.SH 退市复牌 −96.4%，ratio≈0.036)，那是真实亏损不是脏价，
    用对称下界(0.1)会误剔造成幸存者偏差；但 −99%+/日(ratio<0.01)即便退市也到不了，必为脏价。
    [修2] 为何仍需下界：脏数据(北交所新三板期 close 卡 0.01 再跳)多数在"跳回真实价"那天是 >10 倍
    【上跳】被上界抓到；但若幻影低价正好是该股缓存最后一天(其后退市/无数据)，没有回跳可抓 →
    只靠上界会放它进 NAV 把净值静默砸到 ~0。下界正是补这个"末日幻影低价无回跳"的洞。
    ...
    bad = ((px["adj_close"] <= 0)
           ...
           | (ratio < min_daily_ratio)).fillna(False)
    if not bad.any():
        return px
    viol = px.loc[bad, ["code", "date", "adj_close"]].assign(prev=prev[bad], ratio=ratio[bad])
    head = "\n".join(
        f"  {r.code} {pd.Timestamp(r.date).date()} adj_close {r.prev:.4g}->{r.adj_close:.4g} (×{r.ratio:.4g})"
        for r in viol.sort_values("ratio", ascending=False).head(20).itertuples())
    summary = (f"价格质量校验：检出 {int(bad.sum())} 处物理不可能的单日复权价跳变"
               f"（>{max_daily_ratio:g} 倍 / <{min_daily_ratio:g}(跌>99%) / 非正价），"
               f"涉及 {viol['code'].nunique()} 只股。最离谱前 20：\n{head}")
    if on_bad == "raise":
